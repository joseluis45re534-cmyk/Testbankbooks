  ...
  app.get("/api/download/:signedToken", async (req, res) => {
    try {
      ...
      if (product.downloadPath) {
        ...
        if (fileUrl && !fileUrl.startsWith("http") && !fs.existsSync(path.resolve(process.cwd(), fileUrl.replace(/^\//, "")))) {
          fileUrl = null;
        }
      }
      ...
      if ((senderType || "visitor") === "visitor") {
        try {
          ...
          if (shouldBotReply(recent)) {
            ...
            const reply = await generateBotReply(message, visitorEmail, storage);
            await storage.createMessage({
              conversationId,
              message: reply,
              senderType: "bot",
