...
export async function registerRoutes(
  ...
  app.get("/feed/google-shopping.xml", async (req, res) => {
    try {
      ...
      xml += '<channel>\n';
      xml += `  <title>Testbankbooks - Nursing Exam Prep &amp; Study Materials</title>\n`;
      xml += `  <link>${baseUrl}</link>\n`;
      xml += `  <description>Premium nursing exam preparation materials and study resources</description>\n`;
      ...
      for (const product of products) {
        ...
        xml += `    <g:product_type>${escapeXml(product.category || "Exam Prep Materials")}</g:product_type>\n`;
